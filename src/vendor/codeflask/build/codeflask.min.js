!(function (e, t) {
      ...
      const n = (function () {
        ...
        var a = (t.Prism = {
            ...
            clone(e, t) {
              ...
              switch (((t = t || {}), n)) {
                case 'Object':
                  ...
                  var s = {};
                  for (const i in ((t[a.util.objId(e)] = s), e))
                    e.hasOwnProperty(i) && (s[i] = a.util.clone(e[i], t));
                  return s;
                case 'Array':
                  if (t[a.util.objId(e)]) return t[a.util.objId(e)];
                  s = [];
          ...
          languages: {
            ...
            insertBefore(e, t, n, s) {
              const i = (s = s || a.languages)[e];
              if (arguments.length == 2) {
                for (var r in (n = arguments[1]))
            ...
            DFS(e, t, n, s) {
              for (const i in ((s = s || {}), e))
                e.hasOwnProperty(i) &&
                  (t.call(e, i, e[i], n || i),
                  a.util.type(e[i]) !== 'Object' || s[a.util.objId(e[i])]
                    ? a.util.type(e[i]) !== 'Array' ||
                      s[a.util.objId(e[i])] ||
                      ((s[a.util.objId(e[i])] = !0),
                      a.languages.DFS(e[i], t, i, s))
                    : ((s[a.util.objId(e[i])] = !0),
                      a.languages.DFS(e[i], t, null, s)));
            },
          },
          plugins: {},
          highlightAll(e, t) {
          ...
          highlightAllUnder(e, t, n) {
            ...
            for (
              var i, r = s.elements || e.querySelectorAll(s.selector), o = 0;
              (i = r[o++]);

            )
              a.highlightElement(i, !0 === t, s.callback);
          },
          highlightElement(n, s, i) {
          ...
          highlight(e, t, n) {
            const i = { code: e, grammar: t, language: n };
            return (
              a.hooks.run('before-tokenize', i),
              (i.tokens = a.tokenize(i.code, i.grammar)),
              a.hooks.run('after-tokenize', i),
              s.stringify(a.util.encode(i.tokens), i.language)
            );
          },
          matchGrammar(e, t, n, s, i, r, o) {
            const l = a.Token;
            for (const c in n)
              if (n.hasOwnProperty(c) && n[c]) {
                ...
                for (let d = 0; d < u.length; ++d) {
                  ...
                  if (f && !h.pattern.global) {
                    const y = h.pattern.toString().match(/[imuy]*$/)[0];
                    h.pattern = RegExp(h.pattern.source, `${y}g`);
                  }
                  h = h.pattern || h;
                  for (let k = s, v = i; k < t.length; v += t[k].length, ++k) {
                    let x = t[k];
                    if (t.length > e.length) return;
                    if (!(x instanceof l)) {
                      if (f && k != t.length - 1) {
                        if (((h.lastIndex = v), !(L = h.exec(e)))) break;
                        for (
                          var w = L.index + (g ? L[1].length : 0),
                            C = L.index + L[0].length,
                            S = k,
                            T = v,
                            A = t.length;
                          S < A && (T < C || (!t[S].type && !t[S - 1].greedy));
                          ++S
                        )
                          w >= (T += t[S].length) && (++k, (v = T));
                        if (t[k] instanceof l) continue;
                        (F = S - k), (x = e.slice(v, T)), (L.index -= v);
                      } else {
                        h.lastIndex = 0;
                        var L = h.exec(x);
                        var F = 1;
                      ...
                      if (L) {
                        g && (m = L[1] ? L[1].length : 0);
                        C = (w = L.index + m) + (L = L[0].slice(m)).length;
                        const E = x.slice(0, w);
                        const _ = x.slice(C);
                        const N = [k, F];
                        E && (++k, (v += E.length), N.push(E));
                        const j = new l(c, p ? a.tokenize(L, p) : L, b, L, f);
                        if (
                          (N.push(j),
                          _ && N.push(_),
                          Array.prototype.splice.apply(t, N),
                          F != 1 && a.matchGrammar(e, t, n, k, v, !0, c),
                          r)
                        )
                          break;
                      } else if (r) break;
        ...
        if (
          ((s.stringify = function (e, t, n) {
            ...
            if (e.alias) {
              const r = a.util.type(e.alias) === 'Array' ? e.alias : [e.alias];
            ...
            ? (a.disableWorkerMessageHandler ||
                t.addEventListener(
                  ...
                  function (e) {
                    ...
                    t.postMessage(a.highlight(i, a.languages[s], s)),
                      r && t.close();
                  },
                  !1
                ),
              ...
              .forEach(function (t) {
                for (
                  var a,
                    ...
                    i = t,
                    r = /\blang(?:uage)?-([\w-]+)\b/i;
                  i && !r.test(i.className);

                )
                  i = i.parentNode;
                if ((i && (a = (t.className.match(r) || [, ''])[1]), !a)) {
                  const o = (s.match(/\.(\w+)$/) || [, ''])[1];
                  a = e[o] || o;
                }
  ...
  return (
    (d.prototype.startEditor = function () {
      if (!i(s, null, this.opts.styleParent))
    ...
    (d.prototype.runOptions = function () {
      (this.opts.rtl = this.opts.rtl || !1),
        ...
        typeof this.opts.handleTabs !== 'boolean' &&
          (this.opts.handleTabs = !0),
        ...
        if (a !== s && o.length >= c.length) {
          ...
          if (e.shiftKey)
            i.substr(u, c.length) === c
              ? ((d = -d),
                u > a
                  ? ((o = o.substring(0, u) + o.substring(u + c.length)),
                    (h = 0))
                  : u === a
                  ? ((d = 0), (h = 0), (o = o.substring(c.length)))
                  : ((h = -h),
                    (r = r.substring(0, u) + r.substring(u + c.length))))
              : ((d = 0), (h = 0)),
              (o = o.replace(
                new RegExp(`\n${c.split('').join('\\')}`, 'g'),
                '\n'
            ...
            (r = r.substr(0, u) + c + r.substring(u, a)),
              (o = o.replace(/\n/g, `\n${c}`));
          (t.value = r + o + l),
            (t.selectionStart = a + d),
            (t.selectionEnd = a + o.length + h),
    ...
    (d.prototype.handleNewLineIndentation = function (e) {
      if (this.opts.handleNewLineIndentation && e.keyCode === 13) {
        ...
        const s = t.value;
        const i = s.substr(0, n);
        const r = s.substring(a);
        const o = s.lastIndexOf('\n', n - 1);
        const l = o + s.slice(o + 1).search(/[^ ]|$/);
        const c = l > o ? l - o : 0;
        const u = `${i}\n${' '.repeat(c)}${r}`;
        (t.value = u),
    ...
    (d.prototype.closeCharacter = function (e) {
      ...
      if (this.skipCloseChar(e)) {
        ...
        const s = a ? n + 1 : n;
        const i = !a && ["'", '"'].includes(e) ? e : '';
        const r = `${this.code.substring(0, t)}${i}${this.code.substring(s)}`;
        this.updateCode(r),
        ...
        const c = `${this.code.substring(0, t)}${l}${o}${this.code.substring(
          n
        )}`;
        this.updateCode(c);
      }

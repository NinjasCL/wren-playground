...
describe('CodeFlask Tests', () => {
  ...
  it('.onUpdate(): should execute callback upon user interaction', function () {
    $('.codeflask__textarea').setValue('');
    browser.execute(() => {
      flask.onUpdate((code) => (document.title = code));
    });
    $('.codeflask__textarea').setValue('let it = "go";');
    const title = browser.getTitle();
  ...
  it('should add an aria-labelledby attribute with option', function () {
    ...
    expect($('.codeflask__textarea[aria-labelledby="thing2"]').isExisting()).to
      .be.true;
  });
  ...
  it('should not add line by press enter when editor is readonly', function () {
    browser.execute(() => {
      ...
      rootEl
        .querySelector('.codeflask .codeflask__textarea')
        .dispatchEvent(
  ...
  it.skip('should handle the tab key in the editor', function () {
    let flask_test;
    browser.execute(() => {
      const test_div = document.createElement('div');
      document.body.appendChild(test_div);
  ...
  it.skip('should not handle the tab key in the editor with handleTabs=false option', function () {
    let flask_test;
    browser.execute(() => {
      const test_div = document.createElement('div');
      document.body.appendChild(test_div);
